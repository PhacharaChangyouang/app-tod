...
export default function ForgotPasswordPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setCooldown(Number(response?.cooldownSeconds) || 120);
      setSent(true);
    } catch (err) {
      const raw = String(err?.message || '');
      if (err?.status === 429) {
        setCooldown(Math.max(1, Number(err?.data?.retryAfterSeconds) || 120));
        setError('คุณทำรายการเกินจำนวนที่กำหนด กรุณาลองใหม่ในภายหลัง');
      } else if (/Email service is not configured|RESEND_API_KEY|MAIL_FROM/i.test(raw)) {
