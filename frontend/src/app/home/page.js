...
function Brand() {
  return (
    <div className="aha-v3-brand">
      <span className="aha-v3-brand-mark"><svg viewBox="0 0 60 42" fill="none" aria-hidden="true"><path d="M3 25C12 25 12 5 21 5s9 30 18 30 9-25 18-25" stroke="#159FE0" strokeWidth="8" strokeLinecap="round" /><path d="M40 29c7 0 8-13 17-13" stroke="#22B8AA" strokeWidth="8" strokeLinecap="round" /></svg></span>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([dataPromise, notificationApi.unread()]).then(([r, n]) => {
      if (!mounted) return;
      if (r.status === 'fulfilled') {
        if (currentUser?.role === 'caregiver') setCaregiverSummary(r.value?.data || { elderly: [], today: [], history: [] });
        else setReminders(listOf(r.value, 'reminders'));
      }
  ...
  const caregiverName = user?.name || 'ผู้ดูแล';
  ...
  const startVoice = () => {
    ...
    const recognition = new Recognition(); recognition.lang = 'th-TH'; recognition.interimResults = false; setVoice(true);
    ...
    <div className="aha-v3-page">
      <div className="aha-v3-layout"><div className="aha-v3-main">
        <header className="aha-v3-topbar">
          <Brand />
          <button className={`aha-v3-voice-pill ${voice ? 'listening' : ''}`} onClick={startVoice}><span><AhaIcon name="mic" size={23} /></span><strong>{voice ? 'กำลังฟัง…' : 'พูดกับ AHA'}</strong></button>
          ...
          <div className="aha-v3-top-actions">
            <button onClick={() => router.push('/notifications')} aria-label="แจ้งเตือน"><AhaIcon name="bell" size={22} />{notifications.length > 0 && <b>{notifications.length > 9 ? '9+' : notifications.length}</b>}</button>
            <button onClick={logout} aria-label="ออกจากระบบ"><span className="aha-v3-user-avatar">{displayName.slice(0,1)}</span></button>
        ...
        <main className="aha-v3-content">
          ...
          <section className="aha-v3-hero">
            <div className="aha-v3-hero-copy">
              <h1>{isCaregiver ? `สวัสดี ${caregiverName}` : `สวัสดี ${displayName}`}</h1>
              <p>{isCaregiver ? 'วันนี้คุณกำลังดูแลใครอยู่บ้าง?' : 'ดูแลตัวเองไปด้วยกันในทุกวัน'}</p>
              <span className="aha-v3-hero-line" />
            </div>
            <div className="aha-v3-hero-photo"><img src="/elderly-hero.jpg" alt="ผู้สูงอายุ" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = '/elderly-hero.svg'; }} /><div className="aha-v3-hero-note">{isCaregiver ? <>ดูแลคนที่รัก<br />ติดตามยา<br /><b>ได้ทุกวัน</b></> : <>ดูแลตัวเอง<br />ไปด้วยกัน<br /><b>ในทุกวัน</b></>}</div></div>
          </section>
          ...
          {isCaregiver ? (
            <>
              <section className="aha-v3-feature-row">
                <article className="aha-v3-card aha-v3-next">
                  <div className="aha-v3-card-title">ผู้สูงอายุที่ดูแล <button onClick={() => router.push('/family')}>จัดการ <AhaIcon name="arrow" size={15} /></button></div>
                  <div className="aha-v3-next-inner"><span className="aha-v3-clock"><AhaIcon name="users" size={42} /></span><div><strong>{caregiverPeople.length}</strong><span>คนที่เชื่อมต่อแล้ว</span><small>เลือกดูตารางยาและสถานะการทานยา</small></div></div>
                  <button className="aha-v3-primary-button" onClick={() => router.push('/family')}><AhaIcon name="users" size={18} /> ดูแดชบอร์ดผู้ดูแล</button>
                </article>
                <div className="aha-v3-action-stack">
                  <button className="aha-v3-action blue" onClick={() => router.push('/family')}><span className="aha-v3-action-icon"><AhaIcon name="users" size={33} /></span><strong>ติดตามผู้สูงอายุ</strong><span className="aha-v3-arrow">›</span></button>
                  <button className="aha-v3-action blue" onClick={startVoice}><span className="aha-v3-action-icon"><AhaIcon name="mic" size={33} /></span><strong>{voice ? 'กำลังฟัง…' : 'พูดกับ AHA'}</strong><span className="aha-v3-arrow">›</span></button>
                  <button className="aha-v3-action red" onClick={() => router.push('/emergency')}><span className="aha-v3-action-icon"><AhaIcon name="phone" size={33} /></span><strong>ฉุกเฉิน SOS</strong><span className="aha-v3-arrow">›</span></button>
                </div>
              </section>
            ...
            <>
              ...
              <section className="aha-v3-today-card"><div className="aha-v3-section-title"><h2>รายการยาวันนี้</h2><button onClick={()=>router.push('/reminders')}>ดูทั้งหมด <AhaIcon name="arrow" size={17}/></button></div>{loading?<div className="aha-v3-empty">กำลังโหลดข้อมูล…</div>:!activeReminders.length?<div className="aha-v3-empty">ยังไม่มีรายการยา <button onClick={()=>router.push('/reminders')}>+ เพิ่มรายการยา</button></div>:<div className="aha-v3-table">{activeReminders.slice(0,4).map((item,index)=>{const done=item.completed||item.taken||item.is_taken;return <div className="aha-v3-med-row" key={item.id||index}><time>{timeOf(item.reminder_time)}</time><span className={`aha-v3-dot ${done?'done':index===0?'current':''}`}/><div><strong>{item.medicine_name}</strong><small>{item.dosage||'1 รายการ'}</small></div><div className="aha-v3-row-actions"><StatusPill done={done} current={index===0}/>{!done&&<button className="aha-v3-take-button" onClick={async()=>{try{await reminderApi.markTaken(item.id);setReminders(rows=>rows.map(row=>row.id===item.id?{...row,taken:true,completed:true}:row));}catch(err){setError(err.message||'บันทึกการกินยาไม่สำเร็จ');}}}>กินแล้ว</button>}</div></div>})}</div>}</section>
            </>
          )}
