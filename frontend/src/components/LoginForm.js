      ...
      <section className="aha-auth-form-panel">
        ...
        <form onSubmit={submit}>
          ...
          {mode==='register'&&<>
            ...
            <div className="aha-auth-field-row"><div className="aha-auth-field"><label>รหัสผ่าน</label><div className="aha-auth-password-wrap"><input type={showRegisterPassword?'text':'password'} value={register.password} onChange={e=>updateRegister('password',e.target.value)} placeholder="อย่างน้อย 8 ตัว" required/><button type="button" className="aha-auth-password-toggle" onClick={()=>setShowRegisterPassword(v=>!v)} aria-label={showRegisterPassword?'ซ่อนรหัสผ่าน':'แสดงรหัสผ่าน'} aria-pressed={showRegisterPassword}>{showRegisterPassword?'ซ่อน':'แสดง'}</button></div></div><div className="aha-auth-field"><label>ยืนยันรหัสผ่าน</label><div className="aha-auth-password-wrap"><input type={showConfirmPassword?'text':'password'} value={register.confirmPassword} onChange={e=>updateRegister('confirmPassword',e.target.value)} required/><button type="button" className="aha-auth-password-toggle" onClick={()=>setShowConfirmPassword(v=>!v)} aria-label={showConfirmPassword?'ซ่อนรหัสผ่าน':'แสดงรหัสผ่าน'} aria-pressed={showConfirmPassword}>{showConfirmPassword?'ซ่อน':'แสดง'}</button></div></div></div>
            ...
            <div className="aha-auth-field"><label>PIN 4 หลัก</label><input type="password" inputMode="numeric" maxLength={4} value={register.pin} onChange={e=>updateRegister('pin',e.target.value.replace(/\D/g,'').slice(0,4))} placeholder="••••" required/></div>
          ...
          <button className="aha-auth-submit" disabled={busy}>{busy?'กำลังดำเนินการ…':mode==='password'?'เข้าสู่ระบบ':mode==='pin'?'เข้าสู่ระบบด้วย PIN':mode==='otp'?(otpStep==='phone'?'ส่งรหัส OTP':'ยืนยันและเข้าสู่ระบบ'):'สมัครสมาชิก'}</button>
        </form>
        {mode==='otp'&&otpStep==='code'&&<button type="button" className="aha-auth-ghost" onClick={()=>setOtpStep('phone')}>เปลี่ยนเบอร์โทรศัพท์</button>}
      </section>
    </section>
