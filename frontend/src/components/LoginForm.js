...
import PasswordRequirements, { isValidPassword } from './PasswordRequirements';
...
export default function LoginForm({ onSubmit }) {
  ...
  const [googleDetails,setGoogleDetails]=useState({phone:'',pin:'',confirmPin:'',role:'elderly',age:'',termsAccepted:false});
  ...
  const run=async(payload)=>{setError('');setBusy(true);try{return await onSubmit({...payload,setError});}finally{setBusy(false);}};
  ...
  const validateRegistration=()=>{const r=register;
    ...
    if(!isValidPassword(r.password))return'รหัสผ่านต้องมีอย่างน้อย 8 ตัว พร้อมตัวพิมพ์ใหญ่ ตัวพิมพ์เล็ก และตัวเลข';
  ...
  const submit=(e)=>{e.preventDefault();
    if(googleSignup){
      if(!/^0\d{9}$/.test(googleDetails.phone))return setError('เบอร์โทรศัพท์ต้องเป็นตัวเลข 10 หลักและขึ้นต้นด้วย 0');
      if(!/^\d{4}$/.test(googleDetails.pin)||googleDetails.pin!==googleDetails.confirmPin)return setError('PIN ต้องเป็นตัวเลข 4 หลักและตรงกันทั้งสองช่อง');
      if(googleDetails.role==='elderly'&&(!googleDetails.age||Number(googleDetails.age)<1||Number(googleDetails.age)>120))return setError('กรุณากรอกอายุ 1–120 ปี');
      if(!googleDetails.termsAccepted)return setError('กรุณายอมรับเงื่อนไขการใช้งานและนโยบายข้อมูลส่วนบุคคล');
      return run({mode:'googleComplete',signupToken:googleSignup.signupToken,...googleDetails});
    }
  ...
  const googleLogin=async(credential)=>{const result=await run({mode:'google',credential});if(result?.requiresCompletion)setGoogleSignup(result);};
  const updateGoogle=(key,value)=>setGoogleDetails((current)=>({...current,[key]:value}));
  ...
  return <main className="aha-auth-page">
    <section className="aha-auth-card">
      ...
      <section className="aha-auth-form-panel">
        <div className="aha-auth-heading"><div className="aha-auth-kicker">AHA</div><h2>{googleSignup?'กรอกข้อมูล AHA ให้ครบ':mode==='register'?'สร้างบัญชี AHA':'ยินดีต้อนรับกลับ'}</h2><p>{googleSignup?`ยืนยัน Google แล้ว: ${googleSignup.profile?.email || ''}`:mode==='register'?'กรอกข้อมูลเพื่อเริ่มใช้งาน':'เข้าสู่ระบบเพื่อดูข้อมูลและการแจ้งเตือนของคุณ'}</p></div>

        {!googleSignup&&mode!=='register'&&<div className="aha-auth-tabs" role="tablist"><button type="button" className={mode==='password'?'active':''} onClick={()=>changeMode('password')}>บัญชี / รหัสผ่าน</button><button type="button" className={mode==='pin'?'active':''} onClick={()=>changeMode('pin')}>เบอร์โทร / PIN</button></div>}
        {!googleSignup&&mode==='register'&&<button type="button" className="aha-auth-back" onClick={backToLogin}>← กลับไปเข้าสู่ระบบ</button>}

        <form onSubmit={submit}>
          {googleSignup&&<>
            <div className="aha-google-profile"><span>{googleSignup.profile?.picture?<Image src={googleSignup.profile.picture} alt="" width={48} height={48} unoptimized referrerPolicy="no-referrer"/>:(googleSignup.profile?.name||'G').slice(0,1)}</span><div><strong>{googleSignup.profile?.name||'บัญชี Google'}</strong><small>{googleSignup.profile?.email}</small></div></div>
            <div className="aha-auth-field"><label>เบอร์โทรศัพท์</label><input type="tel" inputMode="numeric" value={googleDetails.phone} onChange={e=>updateGoogle('phone',e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="0XXXXXXXXX" required/></div>
            <div className="aha-auth-field-row"><div className="aha-auth-field"><label>ตั้ง PIN 4 หลัก</label><input type="password" inputMode="numeric" maxLength={4} value={googleDetails.pin} onChange={e=>updateGoogle('pin',e.target.value.replace(/\D/g,'').slice(0,4))} required/></div><div className="aha-auth-field"><label>ยืนยัน PIN</label><input type="password" inputMode="numeric" maxLength={4} value={googleDetails.confirmPin} onChange={e=>updateGoogle('confirmPin',e.target.value.replace(/\D/g,'').slice(0,4))} required/></div></div>
            <div className="aha-auth-field"><label>ประเภทบัญชี</label><div className="aha-auth-role-grid"><button type="button" className={googleDetails.role==='elderly'?'active':''} onClick={()=>updateGoogle('role','elderly')}><AhaIcon name="heart" size={19}/><span><strong>ผู้สูงอายุ</strong><small>ติดตามยาและสุขภาพ</small></span></button><button type="button" className={googleDetails.role==='caregiver'?'active':''} onClick={()=>updateGoogle('role','caregiver')}><AhaIcon name="users" size={19}/><span><strong>ผู้ดูแล</strong><small>ดูแลผู้สูงอายุ</small></span></button></div></div>
            {googleDetails.role==='elderly'&&<div className="aha-auth-field"><label>อายุ</label><input type="number" min="1" max="120" value={googleDetails.age} onChange={e=>updateGoogle('age',e.target.value)} required/></div>}
            <label className="aha-auth-terms"><input type="checkbox" checked={googleDetails.termsAccepted} onChange={e=>updateGoogle('termsAccepted',e.target.checked)} required/><span>ฉันยอมรับ <Link href="/terms-of-service" target="_blank">ข้อกำหนดการใช้บริการ</Link> และรับทราบ <Link href="/privacy-policy" target="_blank">นโยบายความเป็นส่วนตัว</Link></span></label>
          </>}
          {!googleSignup&&<>
          {mode==='password'&&<>
          ...
          {mode==='register'&&<>
            <div className="aha-auth-field-row"><div className="aha-auth-field"><label>ชื่อ</label><input value={register.firstName} onChange={e=>updateRegister('firstName',e.target.value)} maxLength={50} autoComplete="given-name" required/></div><div className="aha-auth-field"><label>นามสกุล</label><input value={register.lastName} onChange={e=>updateRegister('lastName',e.target.value)} maxLength={50} autoComplete="family-name" required/></div></div>
            ...
            <div className="aha-auth-field"><label>อีเมล</label><input type="email" value={register.email} onChange={e=>updateRegister('email',e.target.value)} maxLength={254} autoComplete="email" placeholder="name@example.com" required/></div>
            ...
            <div className="aha-auth-field-row"><div className="aha-auth-field"><label>รหัสผ่าน</label><div className="aha-auth-password-wrap"><input type={showRegisterPassword?'text':'password'} value={register.password} onChange={e=>updateRegister('password',e.target.value)} autoComplete="new-password" placeholder="อย่างน้อย 8 ตัว" required/><button type="button" className="aha-auth-password-toggle" onClick={()=>setShowRegisterPassword(v=>!v)} aria-label={showRegisterPassword?'ซ่อนรหัสผ่าน':'แสดงรหัสผ่าน'} aria-pressed={showRegisterPassword}>{showRegisterPassword?'ซ่อน':'แสดง'}</button></div></div><div className="aha-auth-field"><label>ยืนยันรหัสผ่าน</label><div className="aha-auth-password-wrap"><input type={showConfirmPassword?'text':'password'} value={register.confirmPassword} onChange={e=>updateRegister('confirmPassword',e.target.value)} autoComplete="new-password" required/><button type="button" className="aha-auth-password-toggle" onClick={()=>setShowConfirmPassword(v=>!v)} aria-label={showConfirmPassword?'ซ่อนรหัสผ่าน':'แสดงรหัสผ่าน'} aria-pressed={showConfirmPassword}>{showConfirmPassword?'ซ่อน':'แสดง'}</button></div></div></div>
            <PasswordRequirements value={register.password}/>
            ...
            <label className="aha-auth-terms"><input type="checkbox" checked={register.termsAccepted} onChange={e=>updateRegister('termsAccepted',e.target.checked)} required/><span>ฉันยอมรับ <Link href="/terms-of-service" target="_blank">ข้อกำหนดการใช้บริการ</Link> และรับทราบ <Link href="/privacy-policy" target="_blank">นโยบายความเป็นส่วนตัว</Link> ของ AHA</span></label>
          </>}
          </>}
          {error&&<div className="aha-auth-error" role="alert"><strong>ไม่สามารถดำเนินการได้</strong><span>{error}</span></div>}
          <button className="aha-auth-submit" disabled={busy}>{busy?'กำลังดำเนินการ…':googleSignup?'สร้างบัญชีและเข้าสู่ระบบ':mode==='password'?'เข้าสู่ระบบ':mode==='pin'?'เข้าสู่ระบบด้วย PIN':'สมัครสมาชิก'}</button>
          {!googleSignup&&googleEnabled&&<><div className="aha-auth-divider"><span>หรือ</span></div><GoogleSignInButton onCredential={googleLogin} disabled={busy}/></>}
          {googleSignup&&<button type="button" className="aha-auth-ghost" onClick={()=>{setGoogleSignup(null);setError('');}}>ยกเลิกและเลือกบัญชีอื่น</button>}
        </form>
        {!googleSignup&&<nav className="aha-auth-legal" aria-label="เอกสารทางกฎหมาย"><Link href="/privacy-policy">นโยบายความเป็นส่วนตัว</Link><span aria-hidden="true">•</span><Link href="/terms">ข้อกำหนดการใช้บริการ</Link></nav>}
      </section>
    </section>

    <style jsx>{`
      .aha-auth-page{min-height:100vh;display:grid;place-items:center;padding:22px;background:#f5f8f8;color:#20343c;font-family:var(--font-trirong),"Trirong",serif}
