...
export function CaregiverCalendar() {
  ...
  return <section className={`aha-care-calendar ${open?'is-open':'is-collapsed'}`} aria-label="ปฏิทินการดูแล"><div className="aha-care-calendar-head"><div><span>ตารางการดูแล</span><h2>ปฏิทินการดูแล</h2><p>ตรวจสอบแผนการดูแลและรายการสำคัญประจำวัน</p></div><div className="aha-care-calendar-actions"><div className="aha-care-calendar-badge">📅 <b>{now.getDate()}</b><small>วันนี้</small></div><button type="button" className="aha-care-calendar-toggle" onClick={()=>setOpen(v=>!v)} aria-expanded={open}>{open?'ซ่อน':'แสดง'}</button></div></div>{open&&<div className="aha-care-calendar-body"><div className="aha-care-calendar-month"><strong>{month}</strong><span>วันนี้ · {now.getDate()}</span></div><div className="aha-care-calendar-grid aha-care-week"><b>จ</b><b>อ</b><b>พ</b><b>พฤ</b><b>ศ</b><b>ส</b><b>อา</b></div><div className="aha-care-calendar-grid">{days.map((day,i)=><span key={`${day}-${i}`} className={day===now.getDate()?'today':''}>{day||''}</span>)}</div><div className="aha-care-calendar-legend"><span><i className="dot-blue"/>วันนี้</span><span><i className="dot-orange"/>ติดตามยา</span><span>ดูแลกันทุกวัน</span></div></div>}</section>;
}

export function DashboardShowcase() {
  return <div className="aha-care-showcase"><div className="aha-care-showcase-copy"><span>ภาพรวมการดูแล</span><h2>ข้อมูลสำคัญสำหรับผู้ดูแล</h2><p>ตรวจสอบสถานะการทานยา รายการประจำวัน และข้อมูลที่ต้องติดตามได้จากหน้าเดียว</p></div><div className="aha-care-showcase-cards"><div><b>✓</b><strong>สถานะการทานยา</strong><small>ตรวจสอบรายการที่ยืนยันแล้วและรายการที่รอดำเนินการ</small></div><div><b>◷</b><strong>รายการวันนี้</strong><small>ดูตารางและสิ่งที่ต้องติดตามในวันนี้</small></div><div><b>◎</b><strong>ข้อมูลผู้สูงอายุ</strong><small>เข้าถึงข้อมูลการดูแลที่จำเป็นได้อย่างเป็นระบบ</small></div></div></div>;
}
...
export default function AhaCaregiverEnhancements(){
  ...
  return <><AvatarSync /><style jsx global>{`
    ...
    .aha-care-showcase{width:100%;margin:0;padding:24px 26px;border-radius:22px;background:#fff;color:#20352f;position:relative;overflow:hidden;border:1px solid #d9e5e1;box-shadow:0 8px 24px rgba(31,72,64,.07)}
    .aha-care-showcase:after{content:'';position:absolute;width:220px;height:220px;border-radius:50%;right:-65px;top:-100px;background:rgba(47,125,106,.06);box-shadow:-80px 150px 0 20px rgba(216,169,77,.06)}
    .aha-care-showcase-copy{position:relative;z-index:1;max-width:520px}.aha-care-showcase-copy>span{font-size:10px;font-weight:900;letter-spacing:.08em;color:#2f7d6a}.aha-care-showcase h2{margin:6px 0 8px;font-size:28px;line-height:1.15}.aha-care-showcase h2 em{font-style:normal;color:#20352f}.aha-care-showcase p{margin:0;max-width:560px;font-size:13px;line-height:1.6;color:#687a74}
    .aha-care-showcase-cards{position:relative;z-index:1;display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:20px}.aha-care-showcase-cards div{padding:13px;border-radius:16px;background:#f6f9f8;border:1px solid #e2ebe8}.aha-care-showcase-cards b{display:block;font-size:18px;color:#2f7d6a}.aha-care-showcase-cards strong{display:block;font-size:13px;margin-top:4px}.aha-care-showcase-cards small{display:block;margin-top:3px;font-size:10px;line-height:1.45;color:#72827d}
    ...
    @media(max-width:760px){.aha-care-enhancements{width:calc(100% - 20px);margin:14px auto;gap:12px}.aha-care-showcase{width:100%;padding:20px 18px;border-radius:19px}.aha-care-showcase h2{font-size:23px}.aha-care-showcase p{font-size:11px}.aha-care-showcase-cards{grid-template-columns:1fr;gap:7px}.aha-care-showcase-cards div{padding:10px}.aha-care-showcase-cards small{font-size:9px}.aha-care-calendar{width:100%;padding:16px;border-radius:19px}.aha-care-calendar-actions{gap:6px}.aha-care-calendar-badge{width:52px;height:56px;flex-basis:52px}.aha-care-calendar-toggle{min-width:58px;height:36px}.aha-care-calendar-head h2{font-size:19px}.aha-care-calendar-head p{font-size:11px}.aha-care-calendar-grid span{min-height:clamp(30px,9vw,36px);font-size:clamp(10px,3.2vw,12px)}}
